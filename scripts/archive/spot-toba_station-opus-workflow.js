export const meta = {
  name: 'spot-toba_station-opus',
  description: 'Tight-radius dining sweep for Toba Station (A/B test): discover (opus) + verify (opus)',
  phases: [{ title: 'Discover' }, { title: 'Verify' }],
}
const ANGLES = ["鳥羽駅 鳥羽 グルテンフリー 対応 レストラン 食べログ","鳥羽駅 鳥羽 ヴィーガン ベジタリアン カフェ 食べログ","鳥羽駅 鳥羽 老舗 名店 個人店 食堂 食べログ","鳥羽駅 鳥羽 名物 郷土料理 食べログ","鳥羽駅 鳥羽 カフェ 甘味処 食べログ","鳥羽駅 鳥羽 restaurant gluten free vegan celiac","伊勢うどん 食べログ","手こね寿司 食べログ","海鮮 鳥羽 食べログ"];
...
const BOX = {"latMin":34.47419279279279,"latMax":34.48860720720721,"lngMin":136.83465667886264,"lngMax":136.85214332113736};
const norm = s => (s || '').replace(/[\s　・（）()「」、,.。\-本店店]/g, '').toLowerCase();

const discPrompt = (q, avoid) => `Find genuinely good DINING spots in Toba Station (A/B test), Japan, TIGHTLY within: Toba Station (34.4814,136.8434) within 0.8km. Prefer GF-friendly, vegan/vegetarian, shōjin, and beloved local mom-&-pop places, plus notable local specialties. Search the web for: ${q}
...
const verifyPrompt = c => `Verify ONE Toba Station (A/B test) dining spot for a curated gluten-free/vegan-aware app and produce its full record. NEVER invent — every fact from a page you fetch (WebSearch then WebFetch: Tabelog, official site, blogs). found:false if you can't confirm it exists.
TARGET: ${c.name_ja} | area: ${c.area || '?'} | cuisine: ${c.cuisine || '?'}${c.tabelog_url ? ' | Tabelog: ' + c.tabelog_url : ''}
MUST be within the target area: Toba Station (34.4814,136.8434) within 0.8km. If it's clearly outside that radius, set found:false.
...
while (round < 3 && dry < 1) {
  ...
  const disc = await parallel(ANGLES.map((q, i) => () => agent(discPrompt(q, avoid), { label: 'disc r' + round + ' #' + i, phase: 'Discover', schema: DISCOVERY_SCHEMA, model: 'opus' }).then(r => (r && r.candidates) || []).catch(() => [])));
  let fresh = 0;
  for (const list of disc) for (const c of (list || [])) { const k = norm(c.name_ja); if (!k || seen.has(k)) continue; seen.add(k); queue.push(c); fresh++; }
  log('toba_station r' + round + ': +' + fresh + ' fresh (queue ' + queue.length + ')');
  if (fresh < 5) dry++;
}
log('toba_station discovery done: ' + queue.length + ' candidates over ' + round + ' rounds');

phase('Verify')
const verified = (await parallel(queue.map(c => () => agent(verifyPrompt(c), { label: 'vf:' + c.name_ja.slice(0, 16), phase: 'Verify', schema: VERIFY_SCHEMA, model: 'opus' }).then(r => r ? { lead: c.name_ja, ...r } : null).catch(() => null)))).filter(Boolean);
