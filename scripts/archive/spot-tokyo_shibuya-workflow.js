export const meta = {
  name: 'spot-tokyo_shibuya',
  description: 'Tight-radius dining sweep for Tokyo · Shibuya: discover (haiku) + verify (sonnet)',
  phases: [{ title: 'Discover' }, { title: 'Verify' }, { title: 'Safety recheck' }],
}
const ANGLES = ["渋谷スクランブル交差点 渋谷 東京 グルテンフリー 対応 レストラン 食べログ","渋谷スクランブル交差点 渋谷 東京 ヴィーガン ベジタリアン カフェ 食べログ","渋谷スクランブル交差点 渋谷 東京 老舗 名店 個人店 食堂 食べログ","渋谷スクランブル交差点 渋谷 東京 名物 郷土料理 食べログ","渋谷スクランブル交差点 渋谷 東京 カフェ 甘味処 食べログ","渋谷スクランブル交差点 渋谷 東京 restaurant gluten free vegan celiac","渋谷 ラーメン 名店 食べログ 食べログ","渋谷 ヴィーガン グルテンフリー カフェ 食べログ 食べログ","つけ麺 まぜそば 渋谷 食べログ 食べログ"];
...
const BOX = {"latMin":35.65229279279279,"latMax":35.66670720720721,"lngMin":139.69162954533027,"lngMax":139.70937045466974};
const norm = s => (s || '').replace(/[\s　・（）()「」、,.。\-本店店]/g, '').toLowerCase();

const discPrompt = (q, avoid) => `Find genuinely good DINING spots in Tokyo · Shibuya, Japan, TIGHTLY within: Shibuya Scramble (35.6595,139.7005) within 0.8km. Prefer GF-friendly, vegan/vegetarian, shōjin, and beloved local mom-&-pop places, plus notable local specialties. Search the web for: ${q}
...
const verifyPrompt = c => `Verify ONE Tokyo · Shibuya dining spot for a curated gluten-free/vegan-aware app and produce its full record. NEVER invent — every fact from a page you fetch (WebSearch then WebFetch: Tabelog, official site, blogs). found:false if you can't confirm it exists.
TARGET: ${c.name_ja} | area: ${c.area || '?'} | cuisine: ${c.cuisine || '?'}${c.tabelog_url ? ' | Tabelog: ' + c.tabelog_url : ''}
MUST be within the target area: Shibuya Scramble (35.6595,139.7005) within 0.8km. If it's clearly outside that radius, set found:false.
...
while (round < 4 && dry < 2) {
  ...
  log('tokyo_shibuya r' + round + ': +' + fresh + ' fresh (queue ' + queue.length + ')');
  if (fresh < 5) dry++;
}
log('tokyo_shibuya discovery done: ' + queue.length + ' candidates over ' + round + ' rounds');
