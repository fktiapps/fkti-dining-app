// Generates the Nara discovery + combined verify/content workflow.
// Center: Kintetsu Nara Station (34.6845, 135.8296). Scope: ~2 miles (~3.3km).
import fs from 'fs';

const ANGLES = [
  '奈良 近鉄奈良駅 ヴィーガン ベジタリアン カフェ レストラン 食べログ',
  '奈良 グルテンフリー 対応 レストラン カフェ 食べログ',
  '奈良 ならまち 古民家 カフェ 個人店 食べログ',
  '奈良 柿の葉寿司 老舗 個人店 食べログ',
  '奈良 三輪そうめん にゅうめん 老舗 個人店 食べログ',
  '奈良 大和野菜 郷土料理 個人店 食べログ',
  '奈良 茶粥 朝粥 老舗 食べログ',
  '奈良 東向商店街 もちいどの 定食 食堂 個人店 食べログ',
  '奈良 蕎麦 うどん 手打ち 個人店 老舗 食べログ',
  '奈良 喫茶店 昭和 レトロ 純喫茶 個人 食べログ',
  'Nara vegan vegetarian restaurant Naramachi HappyCow',
  'Nara gluten free friendly restaurant near Kintetsu Nara',
  '奈良 おでん 居酒屋 地元 個人店 食べログ',
  '奈良 ラーメン 町中華 個人店 食べログ',
  '奈良 甘味処 和菓子 老舗 個人店 食べログ',
  '奈良 精進料理 おばんざい 一汁三菜 個人店 食べログ',
];
...
const script = `export const meta = {
  name: 'nara-build',
  description: 'Discover + verify + geocode + write bios/safety for Nara restaurants within ~2mi of Kintetsu Nara Station',
...
const discPrompt = q => \`Find real restaurants in CENTRAL NARA, Japan, within about 2 miles of Kintetsu Nara Station (近鉄奈良駅) — i.e. Nara Park, Tōdai-ji, Kōfuku-ji, Naramachi (ならまち), Higashimuki & Mochiidono arcades, Sanjō-dōri, and around JR Nara Station. We want a mix of: (1) vegan / vegetarian / gluten-free-friendly spots, AND (2) small independent local "mom & pop" places (family/solo-run, small, locally loved — incl. Naramachi machiya cafés, kakinoha-zushi (persimmon-leaf sushi), Miwa sōmen/nyūmen, chagayu (tea porridge), Yamato-vegetable & shōjin cuisine, soba, kissaten).
Search the web for: \${q}
Return up to 8 candidates. For each: name_ja (exact Japanese name), area (district + nearest landmark/station), cuisine, tabelog_url (tabelog.com detail URL if found, else ""). Only real shops you saw in results; prefer genuinely local/independent ones near Kintetsu Nara / Naramachi.\`;
...
log('Nara discovery: ' + queue.length + ' unique candidates (cap 90)')
const candidates = queue.slice(0, 90)
...
return {
  counts: { candidates: candidates.length, verified: verified.length, kept: keep.length },
  kept: keep,
...
`;
fs.writeFileSync('scripts/nara-workflow.js', script);
console.log('wrote scripts/nara-workflow.js | angles:', ANGLES.length, '|', script.length, 'bytes');
