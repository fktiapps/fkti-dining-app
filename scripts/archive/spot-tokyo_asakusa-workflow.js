export const meta = {
  name: 'spot-tokyo_asakusa',
  description: 'Tight-radius dining sweep for Tokyo · Asakusa: discover (haiku) + verify (sonnet)',
  phases: [{ title: 'Discover' }, { title: 'Verify' }, { title: 'Safety recheck' }],
}
const ANGLES = ["浅草寺 浅草 東京 グルテンフリー 対応 レストラン 食べログ","浅草寺 浅草 東京 ヴィーガン ベジタリアン カフェ 食べログ","浅草寺 浅草 東京 老舗 名店 個人店 食堂 食べログ","浅草寺 浅草 東京 名物 郷土料理 食べログ","浅草寺 浅草 東京 カフェ 甘味処 食べログ","浅草寺 浅草 東京 restaurant gluten free vegan celiac","浅草 ラーメン 名店 食べログ 食べログ","浅草 ヴィーガン 精進料理 グルテンフリー 食べログ 食べログ","浅草 老舗 天ぷら 蕎麦 どぜう 名物 食べログ 食べログ"];
...
const BOX = {"latMin":35.70759279279279,"latMax":35.722007207207206,"lngMin":139.78782339406794,"lngMax":139.80557660593203};
const norm = s => (s || '').replace(/[\s　・（）()「」、,.。\-本店店]/g, '').toLowerCase();

const discPrompt = (q, avoid) => `Find genuinely good DINING spots in Tokyo · Asakusa, Japan, TIGHTLY within: Sensōji (Asakusa) (35.7148,139.7967) within 0.8km. Prefer GF-friendly, vegan/vegetarian, shōjin, and beloved local mom-&-pop places, plus notable local specialties. Search the web for: ${q}
...
const verifyPrompt = c => `Verify ONE Tokyo · Asakusa dining spot for a curated gluten-free/vegan-aware app and produce its full record. NEVER invent — every fact from a page you fetch (WebSearch then WebFetch: Tabelog, official site, blogs). found:false if you can't confirm it exists.
TARGET: ${c.name_ja} | area: ${c.area || '?'} | cuisine: ${c.cuisine || '?'}${c.tabelog_url ? ' | Tabelog: ' + c.tabelog_url : ''}
MUST be within the target area: Sensōji (Asakusa) (35.7148,139.7967) within 0.8km. If it's clearly outside that radius, set found:false.
...
while (round < 4 && dry < 2) {
  ...
  log('tokyo_asakusa r' + round + ': +' + fresh + ' fresh (queue ' + queue.length + ')');
  if (fresh < 5) dry++;
}
log('tokyo_asakusa discovery done: ' + queue.length + ' candidates over ' + round + ' rounds');
