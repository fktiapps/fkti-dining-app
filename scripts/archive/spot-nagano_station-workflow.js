export const meta = {
  name: 'spot-nagano_station',
  description: 'Tight-radius dining sweep for Nagano · Station: discover (haiku) + verify (sonnet)',
  phases: [{ title: 'Discover' }, { title: 'Verify' }, { title: 'Safety recheck' }],
}
const ANGLES = ["長野駅 長野 グルテンフリー 対応 レストラン 食べログ","長野駅 長野 ヴィーガン ベジタリアン カフェ 食べログ","長野駅 長野 老舗 名店 個人店 食堂 食べログ","長野駅 長野 名物 郷土料理 食べログ","長野駅 長野 カフェ 甘味処 食べログ","長野駅 長野 restaurant gluten free vegan celiac","信州そば 長野駅 食べログ","ラーメン 長野駅 名店 食べログ","おやき 長野 食べログ","ヴィーガン グルテンフリー 長野駅 食べログ","長野駅 居酒屋 郷土料理 食べログ","長野駅 定食 食堂 食べログ"];
...
const BOX = {"latMin":36.6386954954955,"latMax":36.6477045045045,"lngMin":138.18328598477774,"lngMax":138.19451401522224};
const norm = s => (s || '').replace(/[\s　・（）()「」、,.。\-本店店]/g, '').toLowerCase();

const discPrompt = (q, avoid) => `Find genuinely good DINING spots in Nagano · Station, Japan, TIGHTLY within: Nagano Station (36.6432,138.1889) within 0.5km. Prefer GF-friendly, vegan/vegetarian, shōjin, and beloved local mom-&-pop places, plus notable local specialties. Search the web for: ${q}
...
const verifyPrompt = c => `Verify ONE Nagano · Station dining spot for a curated gluten-free/vegan-aware app and produce its full record. NEVER invent — every fact from a page you fetch (WebSearch then WebFetch: Tabelog, official site, blogs). found:false if you can't confirm it exists.
TARGET: ${c.name_ja} | area: ${c.area || '?'} | cuisine: ${c.cuisine || '?'}${c.tabelog_url ? ' | Tabelog: ' + c.tabelog_url : ''}
MUST be within the target area: Nagano Station (36.6432,138.1889) within 0.5km. If it's clearly outside that radius, set found:false.
...
while (round < 4 && dry < 2) {
  ...
  log('nagano_station r' + round + ': +' + fresh + ' fresh (queue ' + queue.length + ')');
  if (fresh < 5) dry++;
}
log('nagano_station discovery done: ' + queue.length + ' candidates over ' + round + ' rounds');
