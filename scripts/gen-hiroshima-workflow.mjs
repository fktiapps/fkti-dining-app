// Hiroshima discovery + combined verify/content workflow. TWO clusters:
//  - A-Bomb Dome (原爆ドーム 34.3955,132.4536), ~3 mile radius (central Hiroshima)
//  - Miyajima / Itsukushima Shrine (厳島神社 34.2959,132.3198), ~1.5 mile radius
import fs from 'fs';

const ANGLES = [
  '広島 お好み焼き 老舗 個人店 食べログ',
  '広島 平和記念公園 本通 ヴィーガン ベジタリアン カフェ 食べログ',
  '広島 グルテンフリー 対応 レストラン カフェ 食べログ',
  '広島 牡蠣 カキ 老舗 個人店 食べログ',
  '広島つけ麺 つけ麺 個人店 食べログ',
  '広島 中華そば ラーメン 個人店 老舗 食べログ',
  '広島 流川 薬研堀 居酒屋 地元 個人店 食べログ',
  '広島 喫茶店 昭和 レトロ 純喫茶 個人 食べログ',
  '広島 定食 食堂 個人店 老舗 食べログ',
  '広島 蕎麦 うどん 手打ち 個人店 食べログ',
  'Hiroshima vegan vegetarian restaurant Peace Park HappyCow',
  'Hiroshima gluten free okonomiyaki restaurant',
  '宮島 あなご飯 穴子 老舗 個人店 食べログ',
  '宮島 牡蠣 焼き牡蠣 表参道商店街 食べログ',
  '宮島 もみじ饅頭 甘味処 カフェ 食べログ',
  '宮島 厳島神社 ヴィーガン ベジタリアン カフェ 個人店 食べログ',
  '宮島 食堂 定食 老舗 個人店 食べログ',
];
...
const script = `export const meta = {
  name: 'hiroshima-build',
  description: 'Discover + verify + geocode + bios/safety for Hiroshima (3mi from A-Bomb Dome) + Miyajima (1.5mi from Itsukushima Shrine)',
...
const discPrompt = q => \`Find real restaurants in HIROSHIMA, Japan, in EITHER of two areas: (1) central Hiroshima within ~3 miles of the A-Bomb Dome / Peace Memorial Park (原爆ドーム) — Hondori, Kamiyachō, Nagarekawa, Hiroshima Station, Hatchōbori; OR (2) Miyajima island within ~1.5 miles of Itsukushima Shrine (厳島神社) — Omotesandō shopping street and the shrine area. We want a mix of: (1) vegan / vegetarian / gluten-free-friendly spots, AND (2) small independent local "mom & pop" places (family/solo-run, locally loved). Hiroshima specialties: Hiroshima-style okonomiyaki, oysters (kaki), Hiroshima tsukemen; on Miyajima: anago-meshi (conger eel rice), grilled oysters, momiji-manjū.
Search the web for: \${q}
Return up to 8 candidates. For each: name_ja (exact Japanese name), area (district/island + nearest landmark), cuisine, tabelog_url (tabelog.com detail URL if found, else ""). Only real shops seen in results; prefer genuinely local/independent ones.\`;
...
phase('Discover')
const disc = await parallel(ANGLES.map(q => () => agent(discPrompt(q), { label: 'discover', phase: 'Discover', schema: DISCOVERY_SCHEMA }).then(r => (r && r.candidates) || [])))
...
log('Hiroshima discovery: ' + queue.length + ' unique candidates (cap 100)')
const candidates = queue.slice(0, 100)

phase('Verify')
const verified = (await parallel(candidates.map(c => () => agent(verifyPrompt(c), { label: 'vf:' + c.name_ja.slice(0, 16), phase: 'Verify', schema: VERIFY_SCHEMA }).then(r => r ? { lead: c.name_ja, ...r } : null).catch(() => null)))).filter(Boolean)
...
return { counts: { candidates: candidates.length, verified: verified.length, kept: keep.length }, kept: keep,
...
`;
fs.writeFileSync('scripts/hiroshima-workflow.js', script);
console.log('wrote scripts/hiroshima-workflow.js | angles:', ANGLES.length, '|', script.length, 'bytes');
