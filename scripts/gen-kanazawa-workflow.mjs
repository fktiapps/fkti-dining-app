// Generates the Kanazawa discovery + combined verify/content workflow.
// Center: Omichō Market (36.5713, 136.6562). Scope: ~2 miles (~3.3km).
import fs from 'fs';

const ANGLES = [
  '金沢 近江町市場 海鮮 寿司 個人店 食べログ',
  '金沢 ヴィーガン ベジタリアン カフェ レストラン 食べログ',
  '金沢 グルテンフリー 対応 レストラン カフェ 食べログ',
  '金沢おでん 老舗 カウンター 個人店 食べログ',
  '金沢 治部煮 加賀料理 老舗 個人店 食べログ',
  '金沢 片町 香林坊 定食 食堂 個人店 食べログ',
  '金沢 ひがし茶屋街 東山 甘味 カフェ 食べログ',
  '金沢 蕎麦 うどん 手打ち 個人店 老舗 食べログ',
  '金沢駅 近江町 ラーメン 町中華 個人店 食べログ',
  '金沢 喫茶店 昭和 レトロ 純喫茶 個人 食べログ',
  'Kanazawa vegan vegetarian restaurant Omicho HappyCow',
  'Kanazawa gluten free friendly restaurant Omicho market',
  '金沢 おばんざい 居酒屋 地元 個人店 食べログ',
  '金沢 寿司 老舗 カウンター 近江町 個人 食べログ',
  '金沢 和菓子 甘味処 老舗 個人店 食べログ',
  '金沢 中央 長町 尾張町 定食 個人店 老舗 食べログ',
];
...
const script = `export const meta = {
  name: 'kanazawa-build',
  description: 'Discover + verify + geocode + write bios/safety for Kanazawa restaurants within ~2mi of Omichō Market',
...
const discPrompt = q => \`Find real restaurants in CENTRAL KANAZAWA, Japan, within about 2 miles of Omichō Market (近江町市場) — i.e. Kanazawa Station, Kōrinbō, Katamachi, Owarichō, Higashi/Nishi Chaya, Kenroku-en, Nagamachi. We want a mix of: (1) vegan / vegetarian / gluten-free-friendly spots, AND (2) small independent local "mom & pop" places (family/solo-run, small, locally loved — incl. Omichō market counters, Kanazawa oden, Kaga/Jibuni cuisine, sushi, soba, kissaten).
Search the web for: \${q}
Return up to 8 candidates. For each: name_ja (exact Japanese name), area (district + nearest landmark/station), cuisine, tabelog_url (tabelog.com detail URL if found, else ""). Only real shops you saw in results; prefer genuinely local/independent ones near Omichō.\`;
...
log('Kanazawa discovery: ' + queue.length + ' unique candidates (cap 90)')
...
`;
fs.writeFileSync('scripts/kanazawa-workflow.js', script);
console.log('wrote scripts/kanazawa-workflow.js | angles:', ANGLES.length, '|', script.length, 'bytes');
