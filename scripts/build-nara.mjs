import fs from 'fs';
const kept = JSON.parse(fs.readFileSync('data/_nara.json', 'utf8'));

const OMI = [34.6845, 135.8296];
...
const inbox = r => r.lat != null && r.lat >= 34.64 && r.lat <= 34.74 && r.lng >= 135.76 && r.lng <= 135.90;
const norm = s => (s || '').replace(/[\s　・（）()「」、,.。\-本店店]/g, '').toLowerCase();

// cuisine classifier (same vocab as assign-cuisine.mjs)
const RULES = [
  ['unagi', /unagi|鰻|eel|うなぎ/i], ['sushi', /sushi|寿司|鮨|海鮮丼|kaisen|conveyor/i], ['katsu', /tonkatsu|とんかつ|katsudon|カツ|cutlet/i],
  ...
  ['tofu', /豆腐|tofu|湯葉|yuba/i], ['ramen', /ramen|ラーメン|中華そば|町中華|中華料理|chinese|chūka|chuka/i],
  ['udon_soba', /soba|udon|そば|うどん|蕎麦|そうめん|素麺|にゅうめん/i], ['shojin', /shojin|shōjin|精進|temple|buddhist/i],
  ['kaiseki', /kaiseki|会席|懐石|kapp[oōう]|割烹|加賀料理|治部煮|kaga|郷土料理/i],
  ['kissaten', /kissaten|喫茶|純喫茶|coffee|caf[eé]|珈琲|pancake|パンケーキ|コンディトライ/i],
  ['yoshoku', /yoshoku|yōshoku|洋食|western|hamburg|ハンバーグ|オムライス|グリル|grill|bistro|フレンチ|french|italian|pasta|カレー|curry|crepe|クレープ/i],
  ['obanzai', /obanzai|おばんざい|home-?style|家庭料理|健康食|オーガニック|organic|vegan|ヴィーガン|ベジ/i],
  ['donburi', /donburi|丼|どんぶり|rice bowl/i], ['izakaya', /izakaya|居酒屋|sake bar|日本酒バー/i],
  ['shokudo', /shokud|食堂|定食|teishoku|diner|cafeteria|canteen|麺類/i], ['sweets', /sweets|bakery|パン|ケーキ|dessert|パフェ|chocolat|和菓子|甘味|sweet/i],
];
...
for (const r of kept) {
  ...
  let base = slugify(r.name_en) || 'kz', id = 'nara_' + base, n = 2;
  while (usedSlugs.has(id)) id = 'nara_' + base + '_' + n++;
  usedSlugs.add(id);
  ...
  built.push({
    ...
    safety: {
      ...
      positives: cleanAnec(s.positives), confidence: ['high', 'medium', 'low', 'none'].includes(s.confidence) ? s.confidence : 'none', last_checked: '2026-06-22',
    },
    dcp: null,
    cultural_comfort: { level: ['guide_only', 'japanese', 'konnichiwa', 'english'].includes(r.cultural_comfort_level) ? r.cultural_comfort_level : 'konnichiwa', note: r.cultural_comfort_note || 'A local Nara spot; a little Japanese or pointing helps.' },
  });
}

const city = {
  city: 'Nara', subtitle: 'ならまち · ベジ・グルテンフリー', center: OMI,
  bounds: [[34.64, 135.78], [34.73, 135.88]], tz: 'Asia/Tokyo', places: built,
};
...
fs.writeFileSync('data/nara.json', ser(city));

// validate
const check = JSON.parse(fs.readFileSync('data/nara.json', 'utf8'));
...
console.log('built:', built.length, '| skipped:', skipped.length, skipped);
console.log('category:', JSON.stringify(catCount));
console.log('cuisine types:', JSON.stringify(czCount));
console.log('dup ids:', dup.length ? dup : 'none', '| meta leaks:', leaks.length ? leaks : 'NONE');
console.log('all in 2.5mi:', check.places.every(p => hav(OMI[0], OMI[1], p.lat, p.lng) <= 4));
