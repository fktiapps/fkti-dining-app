import fs from 'fs';
const kept = JSON.parse(fs.readFileSync('data/_hiro.json', 'utf8'));
...
const inbox = (lat, lng) => lat != null && lat >= 34.26 && lat <= 34.45 && lng >= 134.0 / 1 - 1.7 && lng <= 132.50; // broad; real filter below
...
const RULES = [
  ...
  ['tofu', /豆腐|tofu|湯葉|yuba/i], ['ramen', /ramen|ラーメン|中華そば|町中華|中華料理|tsukemen|つけ麺|chinese/i],
...
for (const r of kept) {
  ...
  if (isMiyajima) {
    if (dM > 2.6) { [lat, lng] = OMOTE; approx = true; }   // bad island geocode → snap to Omotesandō
...
const city = { city: 'Hiroshima', subtitle: '平和記念 · 宮島 · ベジ／グルテンフリー', center: DOME, bounds: [[34.27, 132.29], [34.42, 132.50]], tz: 'Asia/Tokyo', places: built };
...
console.log('category:', JSON.stringify(catCount), '| cuisine types:', Object.keys(czCount).length);
console.log('Miyajima-tagged:', miya, '| dup ids:', dup.length ? dup : 'none', '| meta leaks:', leaks.length ? leaks : 'NONE');
