// Merge round-2 verified Nara places into the EXISTING data/nara.json (append).
import fs from 'fs';
const kept = JSON.parse(fs.readFileSync('data/_nara_r2.json', 'utf8'));
...
const RULES = [
  ...
  ['kaiseki', /kaiseki|会席|懐石|kapp[oōう]|割烹|加賀料理|治部煮|kaga|郷土料理|大和料理/i],
  ...
  ['obanzai', /obanzai|おばんざい|home-?style|家庭料理|健康食|オーガニック|organic|vegan|ヴィーガン|ベジ|マクロビ|玄米|大和野菜/i],
  ['donburi', /donburi|丼|どんぶり|rice bowl/i], ['izakaya', /izakaya|居酒屋|sake bar|日本酒バー/i],
  ['shokudo', /shokud|食堂|定食|teishoku|diner|cafeteria|canteen|麺類/i], ['sweets', /sweets|bakery|パン|ケーキ|dessert|パフェ|chocolat|和菓子|甘味|sweet|葛|くず/i],
];
...
for (const r of kept) {
  ...
  let base = slugify(r.name_en) || 'nara', id = 'nara_' + base, n = 2;
