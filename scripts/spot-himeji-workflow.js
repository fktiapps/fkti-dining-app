export const meta = {
  name: 'spot-himeji',
  description: 'Tight-radius dining sweep for Himeji: discover (Haiku) + verify (Sonnet)',
  phases: [{ title: 'Discover' }, { title: 'Verify' }],
}
const ANGLES = ["姫路城 姫路駅 大手前通り 姫路 グルテンフリー 対応 レストラン 食べログ","姫路城 姫路駅 大手前通り 姫路 ヴィーガン ベジタリアン カフェ 食べログ","姫路城 姫路駅 大手前通り 姫路 老舗 名店 個人店 食堂 食べログ","姫路城 姫路駅 大手前通り 姫路 名物 郷土料理 食べログ","姫路城 姫路駅 大手前通り 姫路 カフェ 甘味処 食べログ","姫路城 姫路駅 大手前通り 姫路 restaurant gluten free vegan celiac","姫路おでん 食べログ","穴子 あなご 姫路 食べログ","姫路 名物 食べログ","蕎麦 うどん 姫路 食べログ"];
...
const BOX = {"latMin":34.82273963963964,"latMax":34.84346036036036,"lngMin":134.67947802644971,"lngMax":134.7047219735503};
const norm = s => (s || '').replace(/[\s　・（）()「」、,.。\-本店店]/g, '').toLowerCase();

const discPrompt = (q, avoid) => `Find genuinely good DINING spots in Himeji, Japan, TIGHTLY within: Himeji Castle ↔ Station (34.8331,134.6921) within 1.15km. Prefer GF-friendly, vegan/vegetarian, shōjin, and beloved local mom-&-pop places, plus notable local specialties. Search the web for: ${q}
...
const verifyPrompt = c => `Verify ONE Himeji dining spot for a curated gluten-free/vegan-aware app and produce its full record. NEVER invent — every fact from a page you fetch (WebSearch then WebFetch: Tabelog, official site, blogs). found:false if you can't confirm it exists.
TARGET: ${c.name_ja} | area: ${c.area || '?'} | cuisine: ${c.cuisine || '?'}${c.tabelog_url ? ' | Tabelog: ' + c.tabelog_url : ''}
MUST be within the target area: Himeji Castle ↔ Station (34.8331,134.6921) within 1.15km. If it's clearly outside that radius, set found:false.
...
while (round < 3 && dry < 1) {
  ...
  log('himeji r' + round + ': +' + fresh + ' fresh (queue ' + queue.length + ')');
  if (fresh < 5) dry++;
}
log('himeji discovery done: ' + queue.length + ' candidates over ' + round + ' rounds');
